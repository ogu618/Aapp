...
public class PrefDataStore {
    private static jp.ac.meijou.android.s241205045.PrefDataStore instance;
    private final RxDataStore<Preferences> dataStore;
    private PrefDataStore(RxDataStore<Preferences> dataStore) {
        this.dataStore = dataStore;
    }
    public static jp.ac.meijou.android.s241205045.PrefDataStore getInstance(Context context) {
        if (instance == null) {
            var dataStore = new RxPreferenceDataStoreBuilder(
                    context.getApplicationContext(), "settings").build();
            instance = new jp.ac.meijou.android.s241205045.PrefDataStore(dataStore);
        }
        return instance;
    }
